    ...
    $("#page-change-num").keypress(function (e) {
        if (e.which == 13) {
            if (check_positive_integer($("#page-change-num").val())){
                $.cookie('session_files_page_num', Number($("#page-change-num").val()));
...
// 回车提交常用文件列表每页条数配置
function page_upload_enter_listen(){
    $("#page-change-num-upload").keypress(function (e) {
        if (e.which == 13) {
            if (check_positive_integer($("#page-change-num-upload").val())){
                $.cookie('session_server_files_page_num', Number($("#page-change-num-upload").val()));
                server_files_pages_size = Number($("#page-change-num-upload").val());
...
function show_bread_path(path){
    ...
    for (let p=0;p<path_list.length;p++){
        if (p === (path_list.length - 1)){
            bread_html = bread_html + '<li class="breadcrumb-item active"><a>' + DOMPurify.sanitize(path_list[p]) + '</a></li>';
        }
        else{
            item_path = item_path + '/' + DOMPurify.sanitize(path_list[p]);
            bread_html = bread_html + '<li class="breadcrumb-item"><a href="javascript:void(0);" onclick="get_files_list(\'' + item_path + '\', \'' + sessid + '\');" style="text-decoration:none;">' + DOMPurify.sanitize(path_list[p]) + '</a></li>';
        }
    }
...
function get_file_filename_html(file_path, file_type_str, file_name){
    let filename_html = '';
    if (file_type_str === 'folder'){
        filename_html = '<a href="javascript:void(0);" onclick="get_files_list(\'' + DOMPurify.sanitize(file_path) + '\', \'' + sessid + '\');" style="text-decoration:none;">' + DOMPurify.sanitize(file_name) + '</a>';
    }
    else{
...
function get_operate_html(file_path, file_type_str){
    let operate_html = '';
    file_path = DOMPurify.sanitize(file_path);
...
function show_server_files_table(files_list){
    ...
    for (let i=0;i<files_list.length;i++){
        ...
        let operate_html = '<a class="nav-link" href="javascript:void(0)" onclick="upload_server_check(\'' + DOMPurify.sanitize(files_list[i]['server_file_name']) + '\', \'' + $('#upload_file_path').text() + '\', \'' + sessid + '\')">\
        ...
        html = html + '<tr>\
        <td>' + num.toString() + '</td>\
        <td style="color: #206BC4">' + DOMPurify.sanitize(files_list[i]['server_file_name']) + '</td>\
        <td>' + file_size + '</td>\
